...
class MailingListView(ModeratorRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Post.objects.filter(author=self.request.user).prefetch_related(
            'actions__posts', 'actions__keyboard_to_represent',
            'send_to__posts', 'channel', 'author'
        )
...
class MailingCreateView(ModeratorRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        channel = get_channel_by_slug(self.kwargs.get('slug'))
...
class MailingDeleteView(ModeratorRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if self.get_object().is_done:
            messages = self.get_object().sent_messages.all().values('message_id', 'chat_id')
            messages = {message.get('message_id'): message.get('chat_id') for message in messages}
            token = self.get_object().channel.telegram_token
            delete_mailing.delay(list_post_id=messages, token=token)
